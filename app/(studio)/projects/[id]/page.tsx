...
export default function ProjectPage({
  ...
  const handleGenerateImages = async () => {
    if (!currentEpisode?.scenes.length) {
      toast.error("请先进行剧本拆解");
      return;
    }
    ...
    try {
      ...
      const results = res.data.results as Array<{ sceneId: string; localPath?: string; error?: string }>;
      let successCount = 0;
      results.forEach((r) => {
        if (r.localPath) {
      ...
      toast.success(`${successCount}/${results.length} 个首帧生成成功`);
      setActiveStep(3);
    } catch {
      toast.error("首帧生成失败");
    } finally {
  ...
  const handleGenerateAudio = async () => {
    ...
    try {
      ...
      toast.success(`${successCount} 个配音生成成功`);
    } catch {
      toast.error("配音生成失败");
    } finally {
      ...
      <div className="p-4 border-b border-border/50 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5" onClick={handleCreateEpisode}>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    并发生成各分镜首帧（并发数 {process.env.GENERATION_CONCURRENCY ?? 2}，避免限流）
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                ...
                <Card>
                  ...
                  <CardContent className="flex flex-col gap-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {videosDone}/{scenes.length} 个视频完成
                    </p>
                    <Button
                      size="sm"
                ...
                <CardContent className="p-4 flex flex-col gap-2">
                  <div className="text-xs text-muted-foreground space-y-1">
                    <div className="flex justify-between">
                      <span>有首帧的分镜</span>
                      <span>{imagesDone} / {scenes.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>有视频的分镜</span>
                      <span>{videosDone} / {scenes.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>有配音的分镜</span>
                      <span>{scenes.filter((s) => s.localAudio).length} / {scenes.length}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
